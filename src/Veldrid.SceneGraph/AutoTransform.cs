...
namespace Veldrid.SceneGraph
{
    ...
    public class AutoTransform : Transform, IAutoTransform
    {
        ...
        private bool _autoScaleToScreen;
        ...
        public bool AutoScaleToScreen
        {
            ...
            set
            {
                ...
                if (_autoScaleToScreen) CullingActive = false;
            }
        }
        ...
        protected Matrix4x4 ComputeMatrix(INodeVisitor visitor)
        {
            ...
            if (visitor is ICullVisitor cullVisitor)
            {
                ...
                if (AutoScaleToScreen)
                {
                    ...
                    if (AutoScaleTransitionWidthRatio > 0)
                    {
                        if (MinimumScale > 0.0)
                        {
                            ...
                            if (size < k)
                                size = MinimumScale;
                            else if (size < i) size = (float) (a + b * size + c * (size * size));
                        }
                        ...
                        if (MaximumScale < float.MaxValue)
                        {
                            ...
                            if (size > p)
                                size = MaximumScale;
                            else if (size > m) size = (float) (a + b * size + c * (size * size));
                        }
                    }
                    else
                    {
                        if (MinimumScale > 0.0 && size < MinimumScale) size = MinimumScale;

                        if (MaximumScale < float.MaxValue && size > MaximumScale) size = MaximumScale;
                    }
                ...
                {
                    var posToEye = Position - eyePoint;
