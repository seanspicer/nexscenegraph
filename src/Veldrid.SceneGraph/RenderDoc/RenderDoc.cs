...
{
    /// <summary>
    ///     Provides access to RenderDoc's in-application API. Can be used to configure, collect, and save RenderDoc capture
    ///     files,
    ///     and to launch and manage the RenderDoc replay UI application.
    /// </summary>
    internal unsafe class RenderDoc
    {
        private readonly RENDERDOC_API_1_4_0 _api;
        private readonly NativeLibrary _nativeLib;

        private RenderDoc(NativeLibrary lib)
        {
            _nativeLib = lib;
            var getApiFunc = _nativeLib.LoadFunction<pRENDERDOC_GetAPI>("RENDERDOC_GetAPI");
            void* apiPointers;
            var result = getApiFunc(RENDERDOC_Version.API_Version_1_2_0, &apiPointers);
        ...
        ///     modification, as well as initialising buffers with undefined contents to a marker value to catch use of
        ///     uninitialised
        ///     memory.
        ...
        public bool OverlayEnabled
        {
            ...
            set
            {
                var bit = (uint) RENDERDOC_OverlayBits.Enabled;
                if (value)
        ...
        public bool OverlayFrameRate
        {
            ...
            set
            {
                var bit = (uint) RENDERDOC_OverlayBits.FrameRate;
                if (value)
                    _api.MaskOverlayBits(~0u, bit);
                else
                    _api.MaskOverlayBits(~bit, 0);
        ...
        public bool OverlayFrameNumber
        {
            ...
            set
            {
                var bit = (uint) RENDERDOC_OverlayBits.FrameNumber;
                if (value)
                    _api.MaskOverlayBits(~0u, bit);
                else
                    _api.MaskOverlayBits(~bit, 0);
        ...
        public bool OverlayCaptureList
        {
            ...
            set
            {
                var bit = (uint) RENDERDOC_OverlayBits.CaptureList;
                if (value)
                    _api.MaskOverlayBits(~0u, bit);
                else
                    _api.MaskOverlayBits(~bit, 0);
        ...
        public void TriggerCapture(uint numFrames)
        {
            _api.TriggerMultiFrameCapture(numFrames);
        }
        ...
        ///     is a handle to the platform window handle that will be set active. May be null for wildcard
        ///     match.
        /// </param>
        ...
        {
            return _api.IsFrameCapturing() != 0;
        }
        ...
        public bool EndFrameCapture()
        {
            return _api.EndFrameCapture(null, null) != 0;
        }
        ...
        {
            return _api.EndFrameCapture(device.ToPointer(), wndHandle.ToPointer()) != 0;
        }

        /// <summary>
        ///     This function will launch the Replay UI associated with the RenderDoc library injected into the running
        ///     application.
        /// </summary>
        /// <returns>The PID of the replay UI if successful, 0 if not successful.</returns>
        public uint LaunchReplayUI()
        {
            return _api.LaunchReplayUI(1, null);
        }
        ...
        {
            var asciiByteCount = Encoding.ASCII.GetByteCount(args);
            var asciiBytes = stackalloc byte[asciiByteCount + 1];
            fixed (char* argsPtr = args)
            {
                var encoded = Encoding.ASCII.GetBytes(argsPtr, args.Length, asciiBytes, asciiByteCount);
        ...
        {
            var asciiByteCount = Encoding.ASCII.GetByteCount(path);
            var asciiBytes = stackalloc byte[asciiByteCount + 1];
            fixed (char* argsPtr = path)
            {
                var encoded = Encoding.ASCII.GetBytes(argsPtr, path.Length, asciiBytes, asciiByteCount);
                asciiBytes[encoded] = 0;
            }
        ...
        public bool IsTargetControlConnected()
        {
            return _api.IsTargetControlConnected() == 1;
        }
        ...
        {
            return Load(GetLibNames(), out renderDoc);
        }
        ...
        {
            return Load(new[] {renderDocLibPath}, out renderDoc);
        }
        ...
        private static bool Load(string[] renderDocLibPaths, out RenderDoc renderDoc)
        {
            try
            {
                var lib = new NativeLibrary(renderDocLibPaths);
        ...
        private static string[] GetLibNames()
        {
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
            {
                ...
                if (programFiles != null)
                {
                    ...
                    if (File.Exists(systemInstallPath)) paths.Add(systemInstallPath);
                }
