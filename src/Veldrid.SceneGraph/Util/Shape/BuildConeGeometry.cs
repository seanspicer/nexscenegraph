...
namespace Veldrid.SceneGraph.Util.Shape
{
    internal class BuildConeGeometry<T> : GeometryBuilderBase<T> where T : struct, ISettablePrimitiveElement
    {
        internal void Build(IGeometry<T> geometry, ITessellationHints hints, Vector3[] colors, ICone cone)
        {
            ...
            if (colors.Length < 1) throw new ArgumentException("Must provide at least one color for cones");
            ...
            var angleDelta = (float) (2.0f * System.Math.PI / numSegments);
            var texCoordHorzDelta = 1.0f / numSegments;
            var texCoordRowDelta = 1.0f / numRows;
            var hDelta = cone.Height / numRows;
            var rDelta = cone.Radius / numRows;
            ...
            var baser = rDelta;
            var basev = topv - texCoordRowDelta;
            float angle;
            float texCoord;
            ...
            if (hints.CreateBody)
            {
                for (uint rowi = 0;
                    rowi < numRows;
                    ++rowi, topz = basez, basez -= hDelta, topr = baser, baser += rDelta, topv = basev, basev -=
                        texCoordRowDelta)
                {
                    BeginQuadStrip();
