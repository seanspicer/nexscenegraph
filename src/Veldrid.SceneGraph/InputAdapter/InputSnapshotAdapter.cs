...
namespace Veldrid.SceneGraph.InputAdapter
{
    public class InputSnapshotAdapter
    {
        ...
        private readonly HashSet<Key> _pushedKeysThisFrame = new HashSet<Key>();
        ...
        {
            if (KeyMap.TryGetValue(key, out var result)) return result;
        ...
        public IReadOnlyList<IUiEventAdapter> Adapt(InputSnapshot snapshot, float width, float height)
        {
            ...
            else if (IsMouseMove())
            {
                ...
                adaptedEvent.EventType = IsMouseButtonDown()
                    ? IUiEventAdapter.EventTypeValue.Drag
                    : IUiEventAdapter.EventTypeValue.Move;
        ...
        {
            if (_currentlyPressedKeys.Add(key)) _pushedKeysThisFrame.Add(key);
