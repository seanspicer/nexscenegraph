...
namespace Veldrid.SceneGraph.RenderGraph
{
    ...
    public class CullVisitor : NodeVisitor, ICullVisitor
    {
        ...
        private Stack<Matrix4x4> ModelMatrixStack { get; } = new Stack<Matrix4x4>();

        public bool Valid => null != GraphicsDevice;

        private IPolytope CullingFrustum { get; } = Polytope.Create();
        ...
        public int RenderElementCount { get; private set; }
        ...
        public Matrix4x4 GetModelViewInverseMatrix()
        {
            ...
            if (false == canInvert) throw new Exception("ModelView Matrix Cannot be Inverted");
        ...
        public Matrix4x4 GetModelViewProjectionInverseMatrix()
        {
            ...
            if (false == canInvert) throw new Exception("ModelViewProjection Matrix Cannot be Inverted");
        ...
        public override void Apply(INode node)
        {
            ...
            if (needsPop) PipelineStateStack.Pop();
        }
        ...
        public override void Apply(IGeode geode)
        {
            ...
            if (geode.IsCullingActive && IsCulled(bb, ModelMatrixStack.Peek())) return;
...
//            }
        ...
        public override void Apply(IBillboard billboard)
        {
            ...
            for (var i = 0; i < billboard.GetNumChildren(); ++i)
            {
                ...
                var drawablePso = pso;
                if (drawable.HasPipelineState) drawablePso = drawable.PipelineState;
                ...
                foreach (var pset in drawable.PrimitiveSets)
                {
                    ...
                    // 
                    IRenderGroupState renderGroupState = null;
                    if (drawablePso.BlendStateDescription.AttachmentStates.Contains(BlendAttachmentDescription
                        .AlphaBlend))
                        renderGroupState = TransparentRenderGroup.GetOrCreateState(GraphicsDevice, drawablePso,
                            pset.PrimitiveTopology, drawable);
                    else
                        renderGroupState = OpaqueRenderGroup.GetOrCreateState(GraphicsDevice, drawablePso,
                            pset.PrimitiveTopology, drawable);
        ...
        public override void Apply(IDrawable drawable)
        {
            ...
            if (drawable.IsCullingActive && IsCulled(bb, ModelMatrixStack.Peek())) return;
            ...
            foreach (var pset in drawable.PrimitiveSets)
            {
                ...
                if (pso.BlendStateDescription.AttachmentStates.Contains(BlendAttachmentDescription.AlphaBlend))
                    renderGroupState =
                        TransparentRenderGroup.GetOrCreateState(GraphicsDevice, pso, pset.PrimitiveTopology, drawable);
                else
                    renderGroupState =
                        OpaqueRenderGroup.GetOrCreateState(GraphicsDevice, pso, pset.PrimitiveTopology, drawable);

                if (false == renderElementCache.TryGetValue(renderGroupState, out var renderElement))
                {
                    var modMatrix = Matrix4x4.Identity;
                    if (matrixNeedsToUpdate) modMatrix = matrix;
        ...
        private Vector4 ComputePixelSizeVector(IViewport viewport, Matrix4x4 projectionMatrix, Matrix4x4 modelMatrix)
        {
            ...
            var scaleRatio = 0.7071067811f /
                             (float) System.Math.Sqrt(scale_11.LengthSquared() + scale_22.LengthSquared());
