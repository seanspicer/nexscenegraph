...
namespace Examples.Common
{
    public class UpdatingInstancesExampleScene
    {
        public static IGroup Build()
        {
            ...
            var instanceData = new InstanceData[(int) (INSTANCE_COUNT * INSTANCE_COUNT * INSTANCE_COUNT)];
            for (var i = 0; i < INSTANCE_COUNT; ++i)
            for (var j = 0; j < INSTANCE_COUNT; ++j)
            for (var k = 0; k < INSTANCE_COUNT; ++k)
            {
                var xPos = (float) (-INSTANCE_BOUNDS + i * INSTANCE_SPACING);
                var yPos = (float) (-INSTANCE_BOUNDS + j * INSTANCE_SPACING);
                var zPos = (float) (-INSTANCE_BOUNDS + k * INSTANCE_SPACING);

                var scale = 1;

                instanceData[i * (int) (INSTANCE_COUNT * INSTANCE_COUNT) + j * (int) INSTANCE_COUNT + k] =
                    new InstanceData(new Vector3(xPos, yPos, zPos), scale * Vector3.One);
            }

            var cubeInstanceData = VertexBuffer<InstanceData>.Create();
        ...
        internal class UpdateCallback : NodeCallback, INodeCallback
        {
            ...
            private readonly IVertexBuffer<InstanceData> _vb;

            internal UpdateCallback(IVertexBuffer<InstanceData> vb)
            {
                _vb = vb;
                _random = new Random();
            ...
            public override bool Run(IObject obj, IObject data)
            {
                ...
                {
                    if (_idx >= _vb.VertexData.Length) _idx = 0;
