...
namespace Nsg.Viewer.OSXWindow
{
    public unsafe struct MTLDevice
    {
        ...
        public MTLSize maxThreadsPerThreadgroup
        {
            get
            {
                if (ObjectiveCRuntime.UseStret<MTLSize>())
                {
                    return ObjectiveCRuntime.objc_msgSend_stret<MTLSize>(this, sel_maxThreadsPerThreadgroup);
                }
                else
                {
                    return ObjectiveCRuntime.MTLSize_objc_msgSend(this, sel_maxThreadsPerThreadgroup);
                }
            }
        }
        ...
        public MTLLibrary newLibraryWithSource(string source, MTLCompileOptions options)
        {
            ...
            IntPtr library = ObjectiveCRuntime.IntPtr_objc_msgSend(NativePtr, sel_newLibraryWithSource,
                sourceNSS,
                options,
                out NSError error);

            ObjectiveCRuntime.release(sourceNSS.NativePtr);
        ...
        {
            IntPtr library = ObjectiveCRuntime.IntPtr_objc_msgSend(NativePtr, sel_newLibraryWithData, data.NativePtr, out NSError error);
        ...
        {
            IntPtr ret = ObjectiveCRuntime.IntPtr_objc_msgSend(NativePtr, sel_newRenderPipelineStateWithDescriptor,
                desc.NativePtr,
                out NSError error);
        ...
        {
            IntPtr ret = ObjectiveCRuntime.IntPtr_objc_msgSend(NativePtr, sel_newComputePipelineStateWithDescriptor,
                descriptor,
                0,
                IntPtr.Zero,
                out NSError error);
        ...
        public MTLCommandQueue newCommandQueue() => ObjectiveCRuntime.objc_msgSend<MTLCommandQueue>(NativePtr, sel_newCommandQueue);

        public MTLBuffer newBuffer(void* pointer, UIntPtr length, MTLResourceOptions options)
        {
            IntPtr buffer = ObjectiveCRuntime.IntPtr_objc_msgSend(NativePtr, sel_newBufferWithBytes,
                pointer,
                length,
                options);
        ...
        {
            IntPtr buffer = ObjectiveCRuntime.IntPtr_objc_msgSend(NativePtr, sel_newBufferWithLength, length, options);
            return new MTLBuffer(buffer);
        }

        public MTLTexture newTextureWithDescriptor(MTLTextureDescriptor descriptor)
            => ObjectiveCRuntime.objc_msgSend<MTLTexture>(NativePtr, sel_newTextureWithDescriptor, descriptor.NativePtr);

        public MTLSamplerState newSamplerStateWithDescriptor(MTLSamplerDescriptor descriptor)
            => ObjectiveCRuntime.objc_msgSend<MTLSamplerState>(NativePtr, sel_newSamplerStateWithDescriptor, descriptor.NativePtr);

        public MTLDepthStencilState newDepthStencilStateWithDescriptor(MTLDepthStencilDescriptor descriptor)
            => ObjectiveCRuntime.objc_msgSend<MTLDepthStencilState>(NativePtr, sel_newDepthStencilStateWithDescriptor, descriptor.NativePtr);

        public Bool8 supportsTextureSampleCount(UIntPtr sampleCount)
            => ObjectiveCRuntime.bool8_objc_msgSend(NativePtr, sel_supportsTextureSampleCount, sampleCount);

        public Bool8 supportsFeatureSet(MTLFeatureSet featureSet)
            => ObjectiveCRuntime.bool8_objc_msgSend(NativePtr, sel_supportsFeatureSet, (uint)featureSet);

        public Bool8 isDepth24Stencil8PixelFormatSupported
            => ObjectiveCRuntime.bool8_objc_msgSend(NativePtr, sel_isDepth24Stencil8PixelFormatSupported);
