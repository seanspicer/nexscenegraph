...
namespace Nsg.Viewer.OSXWindow
{
    public struct MTLComputeCommandEncoder
    {
        ...
        public void setComputePipelineState(MTLComputePipelineState state)
            => ObjectiveCRuntime.objc_msgSend(NativePtr, sel_setComputePipelineState, state.NativePtr);

        public void setBuffer(MTLBuffer buffer, UIntPtr offset, UIntPtr index)
            => ObjectiveCRuntime.objc_msgSend(NativePtr, sel_setBuffer,
                buffer.NativePtr,
                offset,
                index);

        public unsafe void setBytes(void* bytes, UIntPtr length, UIntPtr index)
            => ObjectiveCRuntime.objc_msgSend(NativePtr, sel_setBytes, bytes, length, index);

        public void dispatchThreadGroups(MTLSize threadgroupsPerGrid, MTLSize threadsPerThreadgroup)
            => ObjectiveCRuntime.objc_msgSend(NativePtr, sel_dispatchThreadgroups0, threadgroupsPerGrid, threadsPerThreadgroup);

        public void dispatchThreadgroupsWithIndirectBuffer(
            MTLBuffer indirectBuffer,
            UIntPtr indirectBufferOffset,
            MTLSize threadsPerThreadgroup)
            => ObjectiveCRuntime.objc_msgSend(NativePtr, sel_dispatchThreadgroups1,
        ...
        public void setTexture(MTLTexture texture, UIntPtr index)
            => ObjectiveCRuntime.objc_msgSend(NativePtr, sel_setTexture, texture.NativePtr, index);

        public void setSamplerState(MTLSamplerState sampler, UIntPtr index)
            => ObjectiveCRuntime.objc_msgSend(NativePtr, sel_setSamplerState, sampler.NativePtr, index);
    }
}